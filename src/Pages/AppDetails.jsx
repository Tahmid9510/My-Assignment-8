...
const AppDetails = () => {
    ...
    const handleAddToInstalled = () => {
        ...
        console.log(excitingList)
        let updatedList = []
        if (excitingList) {
            updatedList = [...excitingList, app]
        }
        else {
        ...
        <div className=' md:p-8 p-4 my-5 md:my-10 mx-auto w-full  bg-gray-100'>
            <div className='md:flex gap-4 border-b-1  border-gray-300 pb-8'>
                ...
                <div className='w-full mx-auto ml-3'>
                    ...
                    <button onClick={handleAddToInstalled} disabled={isInstall} className='btn rounded-xl bg-[#00D390] text-white'>{isInstall ? "Installed" : `Install Now (${size} MB)`}</button>
                </div>
            </div>
